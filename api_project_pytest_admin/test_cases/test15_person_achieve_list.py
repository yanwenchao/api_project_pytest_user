...
class TestPersonAchieveList:
    """个人业绩报表用例类"""
    # 读取个人业绩报表测试用例数据
    read_file = OperationExcel(os.path.join(DATA_DIR,"data.xlsx"),"person_achieve_list")
    cases_data = read_file.read_excel()

    @pytest.mark.parametrize("case",cases_data)
    def test_person_achieve_list(self,case,login_fixture):
        """
        :param case: 测试用例数据
        ...
        case["params"] = replace_data(case["params"],TestPersonAchieveList)
