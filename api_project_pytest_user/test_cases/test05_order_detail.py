...
class TestOrderDetail:
    """保险订单详情用例类"""
    # 读取保险订单详情测试用例数据
    read_file = OperationExcel(os.path.join(DATA_DIR,"data.xlsx"),"order_detail")
    cases_data = read_file.read_excel()

    @pytest.mark.parametrize("case",cases_data)
    def test_order_detail(self,case,order_id_fixture):
        """
        :param case: 测试用例数据
        :param login_fixture: 前置方法同名参数，用来传递参数
        """
        # 参数
        token,TestOrderDetail.order_id = order_id_fixture
        url = conf.get("environment", "base_url") + case["url"]
        case["params"] = replace_data(case["params"],TestOrderDetail)
