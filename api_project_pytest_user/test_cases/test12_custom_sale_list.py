...
class TestCustomSaleList:
    """线索详情营销任务用例类"""
    # 读取线索详情营销任务测试用例数据
    read_file = OperationExcel(os.path.join(DATA_DIR,"data.xlsx"),"custom_sale_list")
    cases_data = read_file.read_excel()

    @pytest.mark.parametrize("case",cases_data)
    def test_custom_sale_list(self,case,custom_id_fixture):
        """
        :param case: 测试用例数据
        :param login_fixture: 前置方法同名参数，用来传递参数
        """
        # 获取参数
        token,TestCustomSaleList.custom_id = custom_id_fixture
        url = conf.get("environment", "base_url") + case["url"]
        case["params"] = replace_data(case["params"],TestCustomSaleList)
