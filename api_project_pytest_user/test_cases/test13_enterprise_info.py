...
class TestEnterpriseInfo:
    """获取机构信息用例类"""
    # 读取获取机构信息测试用例数据
    read_file = OperationExcel(os.path.join(DATA_DIR,"data.xlsx"),"enterprise_info")
    cases_data = read_file.read_excel()

    @pytest.mark.parametrize("case",cases_data)
    def test_enterprise_info(self,case,login_fixture):
        """
        :param case: 测试用例数据
        ...
        case["params"] = replace_data(case["params"],TestEnterpriseInfo)
