...
class TestBatchList:
    """批次列表用例类"""
    # 读取批次列表测试用例数据
    read_file = OperationExcel(os.path.join(DATA_DIR,"data.xlsx"),"batch_list")
    cases_data = read_file.read_excel()

    @pytest.mark.parametrize("case",cases_data)
    def test_batch_list(self,case,login_fixture):
        """
        :param case: 测试用例数据
        ...
        case["params"] = replace_data(case["params"],TestBatchList)
